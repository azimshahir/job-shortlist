...
Steps:
1. From the `scraper/` folder (`cd scraper`) run `python pipeline.py --quiet` (fresh scrape, a few minutes). If the user says "cepat" / "quick" / "from raw", run `python pipeline.py --from-raw --quiet` instead — it re-ranks the last scrape in seconds.
