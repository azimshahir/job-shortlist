...
3. Present, in Malay:
   ...
   - **Recommended** jobs as a table: rank, title (as a link to job_url_direct or job_url), company, source (linkedin/indeed), career family + status, keyword score, semantic score (raw), final score, why selected
   - **Also ranked, not recommended**: the rest of the top 10 with company, source, family, final score and the exact selection_reason, so the user can spot a wrong gate
